# PATCH-13: Mobile Robot 강화학습 학습과 Simulation 검증

- 작성일: 2026-08-15
- 선행 조건: PATCH-05·06, PATCH-10, PATCH-12
- 대상: 향후 `forks/turtlebot3_machine_learning/`, `python/`, `config/rl/mobile/`
...
| PATCH-10 | episode MCAP, manifest, system identification, domain randomization | 학습·평가 데이터 규약 |
...
학습 node가 ROS message type을 직접 결정하지 않는다. action table 결과를 PATCH-11 내부 command로 보내 simulation·Yahboom adapter가 각각 변환한다.
...
| warehouse | PATCH-05 AWS warehouse | 좁은 통로·선반 환경 전이 평가 |
| stress | PATCH-10 범위 경계 | noise·latency·friction 실패 지점 확인 |
...
PATCH-10에서 구한 nominal 값 주변만 바꾼다.
...
2. PATCH-12 결정론적 baseline 이상의 success rate
3. completion time
4. action smoothness
