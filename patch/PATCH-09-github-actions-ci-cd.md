...
현재 자동 검증 범위도 build 중심이다. 두 upstream package에는 이 실습이 바로 실행할 unit test가 정의되어 있지 않고, `python/`, `cpp/` 학습 package는 아직 계획 단계다. 존재하지 않는 test coverage를 CI 성공으로 표현하면 안 된다.
