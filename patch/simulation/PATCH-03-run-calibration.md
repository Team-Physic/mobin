...
`check-calibration-bags.sh`는 calibration 계산 전에 입력 bag이 형식적으로 유효한지만 검사한다.

| 검사 항목 | 하는 일 |
|---|---|
| bag 존재 | `data/bags/` 아래 `metadata.yaml`이 있는 directory만 bag으로 간주 |
| topic/type | 각 bag의 `ros2 bag info`에 `/calib/points`(PointCloud2), `/camera/image_raw`(Image), `/camera/camera_info`(CameraInfo)가 있는지 확인 |
| bag 개수 | 최소 5개 pose bag이 있어야 통과 |

이 script는 calibration을 계산하지 않는다. intensity 값 변화나 point/image 시간 동기화 품질도 여기서는 검사하지 않는다.

`code/scripts/check-calibration-bags.sh`:
...
```bash
chmod +x code/scripts/check-calibration-bags.sh
./code/scripts/check-calibration-bags.sh
```
...
`run-calibration.sh`는 calibration pipeline의 네 단계를 순서대로 Docker container에서 실행한다.

| 단계 | 하는 일 |
|---|---|
| `preprocess` | `check-calibration-bags.sh`를 먼저 실행하고, `direct_visual_lidar_calibration preprocess`로 `/bags`를 읽어 `/results/calib.json`과 중간 결과 생성 |
| `initial` | 이전 단계 결과가 있으면 `initial_guess_manual`로 초기 추정값 저장 |
| `calibrate` | `calibrate`로 `T_lidar_camera` 최적화 |
| `viewer` | 결과가 있으면 calibration viewer 실행 |

script는 알고리즘을 직접 계산하지 않는다. 각 단계에 맞는 container 실행 명령과 인자만 준비하고, 필수 이전 결과가 없으면 중단한다.

`code/scripts/run-calibration.sh`:
...
```bash
chmod +x code/scripts/run-calibration.sh
```

이 image의 `/ros_entrypoint.sh`는 `/root/ros2_ws/install/setup.bash`를 source하므로 `--user`를 쓰면 non-root가 `/root`에 접근하지 못해 `Permission denied`가 난다. 따라서 root로 실행한다. 결과 파일이 root 소유가 되면 host에서 `chown`으로 되돌린다. NVIDIA GPU를 요구하지 않으며 기본값은 Mesa software rendering이다. NVIDIA override가 필요하면 host에서 `LIBGL_ALWAYS_SOFTWARE=0`과 Docker GPU 옵션을 별도로 추가하지만, 첫 실습에서는 CPU 경로로 충분하다.
...
```bash
xhost +local:root
./code/scripts/run-calibration.sh preprocess
```
...
```bash
./code/scripts/run-calibration.sh initial
```
...
```bash
./code/scripts/run-calibration.sh calibrate
```
...
```bash
./code/scripts/run-calibration.sh viewer
```
