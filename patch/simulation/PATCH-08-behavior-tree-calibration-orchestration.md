...
현재 상위 리포에는 실행 가능한 calibration orchestrator가 없다. Simulation PATCH-02는 5개 pose마다 simulation을 다시 실행하고, 센서가 안정되기를 기다린 뒤, 작업자가 `ros2 bag record`를 시작·종료하는 절차를 정의한다. Simulation PATCH-03의 `check-calibration-bags.sh`·`run-calibration.sh`와 Simulation PATCH-04의 `capture-ground-truth.py`·`extrinsic_math.py`는 실행 파일로 구현됐지만, 이 단계를 하나의 상태 기계로 연결하는 orchestrator는 없다.
