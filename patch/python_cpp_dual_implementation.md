...
- 대상: 루트 `python/`, `cpp/`, Docker Compose, PATCH-04, PATCH-06, PATCH-10
- 결론: **공통 Gazebo 환경은 한 번만 유지하고, extrinsic 계산과 장애물 회피를 Python/C++ ROS 2 패키지로 각각 구현·검증한다. 현재 단계는 과정 작성이며 실제 package는 만들지 않는다.**

### Why?

현재 보고서의 구현 위치가 섞여 있다.

- PATCH-04: `scripts/extrinsic_math.py` Python 단일 구현
- PATCH-06: TurtleBot3 fork의 `turtlebot3_drive.cpp` C++ 단일 구현
- 같은 문제를 두 언어로 작성·비교하기 어려움
- 학습 코드와 upstream 코드 경계 불명확

학습 코드를 상위 리포의 `python/`, `cpp/`로 분리한다. SDF, URDF, world, bridge, bag은 공통 자산이므로 복제하지 않는다.

### What I Made

이 문서는 디렉터리 구조, package 생성, Docker mount, 공통 인터페이스, 언어별 구현, 동일 입력 검증 순서를 정의한다. 실제 코드는 후속 구현 단계에서 만든다.
...
```text
# mobile-robot-calibration-repo | planned language split
기존 계획
├── scripts/extrinsic_math.py
└── forks/turtlebot3_simulations/.../turtlebot3_drive.cpp

변경 계획
├── python/mobile_robot_lab_python/  # rclpy 구현
├── cpp/mobile_robot_lab_cpp/        # rclcpp 구현
...
```text
# mobile-robot-calibration-repo | planned repository layout
...
`python/`, `cpp/`는 언어 그룹. 실제 colcon package는 한 단계 아래에 둔다.
...
```text
# python/.../extrinsic_math.py and cpp/.../extrinsic_math.cpp | interface contract
...
```text
# python/.../extrinsic_math.py and cpp/.../extrinsic_math.cpp | transform contract
...
```text
# python/mobile_robot_lab_python | PATCH-04 files
...
```python
# python/mobile_robot_lab_python/setup.py | setup()
entry_points={
    'console_scripts': [
...
```text
# cpp/mobile_robot_lab_cpp | PATCH-04 files
...
```cmake
# cpp/mobile_robot_lab_cpp/CMakeLists.txt | extrinsic targets
...
```text
# python/mobile_robot_lab_python | PATCH-06 files
...
```text
# cpp/mobile_robot_lab_cpp | PATCH-06 files
...
```cmake
# cpp/mobile_robot_lab_cpp/CMakeLists.txt | obstacle target
...
```text
# colcon install space | expected executables
mobile_robot_lab_python extrinsic_math_py
mobile_robot_lab_python obstacle_avoidance_py
mobile_robot_lab_cpp extrinsic_math_cpp
mobile_robot_lab_cpp obstacle_avoidance_cpp
```
...
| PATCH | 반영 내용 |
|---|---|
| PATCH-00 | `python/`, `cpp/` mount와 package build |
